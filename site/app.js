...
// TODO(integration): вписать реальный публичный API-ключ DaData (Suggestions API).
// Получить бесплатный ключ: https://dadata.ru/api/suggest/ (личный кабинет -> "Подключение").
// Ключ используется прямо в браузере — это ожидаемо для прототипа на бесплатном тарифе
// (см. CONTRACT.md §5). Если тариф требует ограничение по домену — настраивается в ЛК DaData.
const DADATA_TOKEN = 'REPLACE-ME-DADATA-TOKEN';
