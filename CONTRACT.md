...
## 1a. `site/course-dates.json`

Расписание запусков/выпусков по группам длительности курса, лежит в `site/course-dates.json`, форма читает его через `fetch('./course-dates.json')` (необязательно — если файл не загрузился, выбор даты у слушателя сводится к пункту «Другие сроки», форму это не блокирует). Обновляется вручную по мере появления новых дат у заказчика — не генерируется скриптом, в отличие от `courses.json`.
...
```json
{
  "buckets": [
    {
      "id": "2w",
      "label": "16, 24, 36, 44 часа — 2 недели",
      "hoursMax": 44,
      "ranges": [
    ...
    {
      "id": "4w",
      "label": "72, 108, 144 часа — 4 недели",
      "hoursMin": 45,
      "hoursMax": 144,
      "ranges": [ { "start": "2026-01-13", "end": "2026-02-12" } ]
    }
  ]
}
```

- `buckets[]` — непересекающиеся группы по часам курса, отсортированы по возрастанию. Курс попадает в первую группу, где `hoursMin <= course.hours <= hoursMax` (`hoursMin` можно не указывать — тогда нижней границы нет, годится для самой младшей группы).
- `ranges[]` — варианты «дата начала – дата окончания» для этой группы часов, ISO `YYYY-MM-DD`. Форма показывает в выпадающем списке слушателя только варианты с `start >= сегодня`, отформатированные как `ДД.ММ.ГГГГ – ДД.ММ.ГГГГ`.
- Группы одинаковы для всех курсов каталога независимо от `category` (подтверждено заказчиком) — деления по категории (Охрана труда/Пожарная безопасность/Первая помощь) в этом файле нет.
- Курс с часами вне всех `buckets` (больше `hoursMax` последней группы) — в списке доступен только пункт «Другие сроки».
- Пункт **«Другие сроки»** — всегда последний в списке независимо от найденной группы; при выборе показывает обычное поле даты (`<input type="date">`), значение уходит в `listeners[].date`, `listeners[].dateEnd` при этом `null`.
...
- `listeners[].date`: строка (`YYYY-MM-DD`) — дата начала обучения. Либо `start` выбранного диапазона из `site/course-dates.json` (см. §1a), либо дата, введённая вручную при выборе «Другие сроки».
- `listeners[].dateEnd`: строка (`YYYY-MM-DD`) или `null` — дата окончания обучения; заполняется только при выборе диапазона из расписания, при «Других сроках» — `null`.
...
- «Дата проведения» — `listeners[].date`, либо `date – dateEnd` через тире, если `dateEnd` заполнен (выбран диапазон из расписания, а не «Другие сроки»).
...
## 3a. POST `site` → `yandex-function` — `POST ?action=link_created`

Шлётся обычным `fetch` (с `keepalive: true`, не `sendBeacon` — вызывается по клику, а не на unload) в момент, когда менеджер нажимает «Скопировать» на сгенерированной ссылке с `?deal=` в блоке «Ссылка для клиента» (см. `site/app.js`, `copyDealLink`/`trackLinkCreated`). Это сигнал того, что ссылка создана и передаётся клиенту на заполнение — отдельная метрика от «Метрики эксперимента» (та считает заполнение самой формы, а не факт выдачи ссылки). Тело:
...
Отправляется не более одного раза на один `dealId` за сессию страницы (защита от повторной записи при повторных кликах на «Скопировать», см. `state.linkCreatedDealIds`). Функция пишет строку в лист «Созданные ссылки» (см. §4.3).
...
```json
{
  ...
  "sheet": "unbound" | "metrics" | "link_created",
  "row": { }
}
```
...
### 4.3 `sheet: "link_created"` → лист «Созданные ссылки»
...
```
Одна строка = одна ссылка, скопированная менеджером для отправки клиенту (см. §3a). Количество ссылок, созданных менеджерами, — просто число строк в этом листе (за нужный период — по колонке `createdAt`).
